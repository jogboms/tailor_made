  ...
  Widget build(BuildContext context) {
    ...
    return Consumer(
      builder: (BuildContext context, WidgetRef ref, Widget? child) => ref
          .watch(selectedContactJobProvider(contactId: payment.contactID, jobId: payment.jobID))
          .when(
              ...
              appBar: AppBar(
                ...
                actions: <Widget>[
                  IconButton(
                    ...
                    onPressed: () => context.registry.get<JobsCoordinator>().toJob(state.selectedJob),
                  ),
                  IconButton(
                    icon: const Icon(Icons.person),
                    onPressed: () => context.registry.get<ContactsCoordinator>().toContact(state.selectedContact.id),
                  ),
